...
rm(list=ls())
datapath='D:/ZZZ/Manuscripts/Gradient_study/lifespan_gradient';
setwd(datapath);

savepath='D:/ZZZ/Manuscripts/Gradient_study/lifespan_gradient/test';


#read the NAWM diffusion data
MRI<-read.xlsx2('Diffusion_value_NAWM_ring_all_diseases_0817test_del_repeat.xls',sheetIndex=1,header= FALSE);Str<-'NDI'
#MRI<-read.xlsx2('Diffusion_value_NAWM_ring_all_diseases_0817test_del_repeat.xls',sheetIndex=3,header= FALSE);Str<-'ODI'

MRI<-MRI[MRI[,1]!=''&MRI[,1]!='[]'&MRI[,1]!='NaN'&MRI[,1]!='NA',]
rownames(MRI)<-MRI[,1]

col1=2;col2=11 #remove the first ring closest to the CSF
Var_index<-col1:col2
...
Clinical<-read.xlsx2('Diffusion_value_NAWM_ring_all_diseases_0817test_del_repeat.xls',sheetIndex=4,header = TRUE);
Clinical<-Clinical[Clinical[,5]!=''&Clinical[,5]!='[]'&Clinical[,5]!='NaN'&Clinical[,5]!='NA',]
rownames(Clinical)<-Clinical[,5]



#select the case with all data
Select_ind0<-intersect(rownames(MRI),rownames(Clinical))

MRI<-MRI[Select_ind0,]
Clinical<-Clinical[Select_ind0,]

#select the case with avalibale groups
Select_ind1<-which(Clinical[,1]>0&Clinical[,1]!='NA'&Clinical[,1]!=2&Clinical[,6]>0)
...
Sub_name<-rownames(data)

data<-data.frame(apply(data,2,as.numeric))
Clinical<-data.frame(apply(Clinical,2,as.numeric))
#using mean value replace the NAN

rownames(data)<-Sub_name
rownames(Clinical)<-Sub_name
...
{ 
   ...
   #using the mean, median value to process the missing values
}
...
{tem_HC<-data[Clinical[,1]=='1',i]
 tem_AD<-data[Clinical[,1]=='3',i]
 tem_PD<-data[Clinical[,1]=='4',i]
 tem_SVD<-data[Clinical[,1]=='5',i]
 tem_MS<-data[Clinical[,1]=='6',i]
...
tem_AD<-data[Clinical[,1]=='3',]
tem_PD<-data[Clinical[,1]=='4',]
tem_SVD<-data[Clinical[,1]=='5',]
tem_MS<-data[Clinical[,1]=='6',]
...
# #just for table 1
# library(psych)
# Clinical[Age<45&Group=='1',1]<-11
# Clinical[Age>=45&Age<60&Group=='1',1]<-12
# Clinical[Age>=60&Group&Group=='1',1]<-13
# 
# #Clinical$Sex..F.1.M.0.<-as.factor(Clinical$Sex..F.1.M.0.)
# 
# describeBy(Clinical[c('Age','Sex..F.1.M.0.', 'TIV', 'GM', 'WM','Lesion_volume',
#                       'GM_ICVF', 'GM_ODI', 'WM_ICVF', 'WM_ODI',
#                       'Lesion_ICVF', 'Lesion_ODI', 'MMSE', 'MOCA','文化程度','病程.y.')],list(Group=Clinical[,1]))
# 
# write.csv(Clinical,file = paste0('clinical_data_for_table1.csv'),quote = T,row.names = T)
# #just for table 1
...
data_cli$New_group<-factor(data_cli$New_group,levels=c('Youth','Middle','Elderly','AD','PD','SVD','MS'))
...
p1 = ggboxplot(data_cli, x = "New_group", y = colnames(data_cli)[1],
  ...
  stat_compare_means(comparisons = list(c('Middle','Youth'),
                                        c('Elderly','Youth'),
                                        c('AD','Youth'),
                                        c('PD','Youth'),
                                        c('SVD','Youth'),
  ...
  
{
  ...
  New_data0[(i-1)*ring_num+j,8]<-Lesion_vol[i]
...
New_data0[,8]<-as.numeric(New_data0[,8])

colnames(New_data0)<-c('value','ring_dist','individual','Group','Age','Sex','Protocol','Lesion')
...
for (i in 1:ncol(data))
{
  data_zscore[,i]<-(data[,i]-mean(data[New_group=='Youth',i]))/sd(data[New_group=='Youth',i])
...
mean_data<-data.frame(matrix(nrow=5,ncol=10))
mean_data[1,]<-colMeans(data_zscore[New_group=='Middle'|New_group=='Elderly',]);
mean_data[2,]<-colMeans(data_zscore[New_group=='AD',]);
mean_data[3,]<-colMeans(data_zscore[New_group=='PD',]);
mean_data[4,]<-colMeans(data_zscore[New_group=='SVD',]);
mean_data[5,]<-colMeans(data_zscore[New_group=='MS',]);

write.csv(mean_data,file = paste0(Str,"_mean_data_for_gene_analysis.csv"),
          quote = F,row.names = F)
  ...
  
{
  ...
  New_data[(i-1)*ring_num+j,8]<-Lesion_vol[i]
...
New_data[,8]<-as.numeric(New_data[,8])

colnames(New_data)<-c('value','ring_dist','individual','Group','Age','Sex','Protocol','Lesion')
write.csv(New_data,file = paste0(Str,"_Newdata_results.csv"),
          quote = F,row.names = F)


#plot before the covariate regressed
New_data$Group<-factor(New_data$Group,levels=c('Youth','Middle','Elderly','AD','PD','SVD','MS'))
...
New_data1$Group<-factor(New_data1$Group,levels=c('Youth','Middle','Elderly','AD','PD','SVD','MS'))
...
LME<-lmer(value~Age+Sex+Protocol+ring_dist+Group+ring_dist*Group+(1+ring_dist|individual),data=New_data)

#nLME<-nlmer(value~sslogis(Age+Sex+ring_dist+Group+ring_dist*Group+(1+ring_dist|individual))~1,data=New_data)


summary(glht(LME, mcp(Group="Tukey")))
summary(glht(LME, mcp(Sex="Tukey")))
summary(glht(LME, mcp(Protocol="Tukey")))

#compared to Youth
Result1<-summary(LME)
#Compared to other Groups
Result2<-summary(glht(LME, linfct=c(
                           'ring_dist:GroupElderly-ring_dist:GroupMiddle=0',
                           ...
                           'ring_dist:GroupAD-ring_dist:GroupElderly=0',
                           'ring_dist:GroupPD-ring_dist:GroupElderly=0',
                           'ring_dist:GroupSVD-ring_dist:GroupElderly=0',
                           'ring_dist:GroupMS-ring_dist:GroupElderly=0',
...
write.csv(Result1,paste0(Str,'_Result1.csv'))
write.csv(Result2,paste0(Str,'Result2.csv'))
...
Cor_Group<-New_group
Cor_data<-cbind(Result4$individual$`(Intercept)`,Result4$individual$ring_dist,Age,Sex,TIV,GM,WM,WM_ICVF,WM_OD,Lesion_vol,MMSE,MOCA,Cor_Group,Protocol)
colnames(Cor_data)<-c('Intercept','Slope','Age','Sex','TIV','GMV','WMV','WM_ICVF','WM_OD','Lesion_volume','MMSE','MOCA','Group','Protocol')
...
fit0<-lm(Slope~Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Youth'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Middle'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Elderly'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Age+Sex,data=Cor_data,subset=(Cor_data$Group=='AD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Age+Sex,data=Cor_data,subset=(Cor_data$Group=='PD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Age+Sex,data=Cor_data,subset=(Cor_data$Group=='SVD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Age+Sex,data=Cor_data,subset=(Cor_data$Group=='MS'))
R1<-summary(fit0)
R2<-confint(fit0)
...
Cor_data$Group<-factor(Cor_data$Group,levels=c('Youth','Middle','Elderly','AD','PD','SVD','MS'))
...
fit0<-lm(Slope~GMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='Youth'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~GMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='Middle'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~GMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='Elderly'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~GMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='AD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~GMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='PD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~GMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='SVD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~GMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='MS'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~WMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='Youth'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~WMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='Middle'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~WMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='Elderly'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~WMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='AD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~WMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='PD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~WMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='SVD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~WMV+Age+Sex+TIV+Protocol,data=Cor_data,subset=(Cor_data$Group=='MS'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Lesion_volume+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Youth'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Lesion_volume+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Middle'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Lesion_volume+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Elderly'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Lesion_volume+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='AD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Lesion_volume+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='PD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Lesion_volume+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='SVD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~Lesion_volume+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='MS'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MOCA+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Youth'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MOCA+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Middle'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MOCA+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Elderly'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MOCA+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='AD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MOCA+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='PD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MOCA+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='SVD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MOCA+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='MS'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MMSE+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Youth'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MMSE+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Middle'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MMSE+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='Elderly'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MMSE+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='AD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MMSE+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='PD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MMSE+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='SVD'))
R1<-summary(fit0)
R2<-confint(fit0)
...
fit0<-lm(Slope~MMSE+Age+Sex,data=Cor_data,subset=(Cor_data$Group=='MS'))
R1<-summary(fit0)
R2<-confint(fit0)
