...
datapath='N:\Life_span_gradient\T1_NODDI';%path to data
savepath='N:\Life_span_gradient\results'%path to save results
cd(datapath);
f=dir;f1=f(3:end);
...
cd(savepath);
tem.fname='New_CSF_group_mask.nii';
...
tem=spm_vol('New_Ventricular_group_level_1.nii');
data=spm_read_vols(tem);
...
tem=spm_vol('N:\Life_span_gradient\results\New_Ventricular_group_level_final.nii');
